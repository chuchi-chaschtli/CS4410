...
fun codegen frame stm =
  let
    ...
    fun munchStm (T.SEQ(x, y)) = (munchStm x; munchStm y)
      ...
      | munchStm (T.MOVE(T.MEM(T.BINOP(T.PLUS, T.CONST n, e1)), e2)) =
        emit (A.OPER {assem="sw `s0, " ^ formatInt n ^ "(`s1)\n",
    ...
    and munchArgs(i, nil) = nil
      | munchArgs(i, argExp::args) =
        let val numArgs = List.length Frame.argregs in
          if i < numArgs then
            let val dest = List.nth(Frame.argregs, i)
                val source = munchExp(argExp)
            in
              munchStm(T.MOVE(T.TEMP dest, T.TEMP(source)));
              dest :: munchArgs(i + 1, args)
            end
          else ErrorMsg.impossible "too many arguments for non-spilling compiler" (* TODO: spilling *)
        end
  in
